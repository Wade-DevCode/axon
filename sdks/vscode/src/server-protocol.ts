...
export class AxonServerProtocolError extends Error {
  constructor(runtimeVersion: string, protocolVersion: unknown) {
    ...
    super(
      `Axon server ${runtimeVersion} uses app-server protocol ${actual}, but the Axon VS Code extension supports protocol ${SUPPORTED_APP_SERVER_PROTOCOL}. Update the Axon CLI or extension so their app-server protocols match.`,
    )

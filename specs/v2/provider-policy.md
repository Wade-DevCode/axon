...
Both `action` and `resource` use axon's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.axon` files is intentionally deferred until `.axon` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether axon permits an operation through its managed execution paths.

...
The axon server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@axon-ai/llm` tool loop after replacing its
...
- plugins get global "axon" instance like in that post i showed
- axon instance has stuff like `axon.session.prompt()` or
  `axon.tool.register({...})`
...
- decide whether to preserve deprecated `@axon-ai/llm` orchestration exports

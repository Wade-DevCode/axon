# Remove `packages/axon/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/axon/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the axon legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/axon/src/storage/db.ts` are concentrated in 22 source files:

- `packages/axon/src/account/repo.ts`
- `packages/axon/src/cli/cmd/db.ts`
- `packages/axon/src/cli/cmd/import.ts`
- `packages/axon/src/cli/cmd/stats.ts`
- `packages/axon/src/control-plane/workspace.ts`
- `packages/axon/src/index.ts`
- `packages/axon/src/node.ts`
- `packages/axon/src/permission/index.ts`
- `packages/axon/src/project/project.ts`
- `packages/axon/src/server/projectors.ts`
- `packages/axon/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/axon/src/server/shared/fence.ts`
- `packages/axon/src/session/message-v2.ts`
- `packages/axon/src/session/projectors.ts`
- `packages/axon/src/session/prompt.ts`
- `packages/axon/src/session/session.ts`
- `packages/axon/src/session/todo.ts`
- `packages/axon/src/share/share-next.ts`
- `packages/axon/src/storage/db.ts`
- `packages/axon/src/sync/index.ts`
- `packages/axon/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy axon database wrapper; `packages/axon/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the axon wrapper.
...
Status: Completed. `SyncEvent` and the axon projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy axon database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy axon database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy axon database wrapper.
...
All migration groups are complete or superseded. `packages/axon/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No axon data-migration group remains.

The previous axon `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate axon data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/axon`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/axon/src`
- `bun typecheck` from `packages/axon`
- Relevant package tests from `packages/axon`, not the repo root

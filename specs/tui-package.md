...
Move the canonical Axon terminal application from
`packages/axon/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/axon`,
`packages/cli`, or `@axon-ai/core`.

The SDK is the TUI's Axon boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/axon`.
- Do not replace private `packages/axon` imports with `@axon-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@axon-ai/tui`.
...
- Axon domain data displayed by the TUI
...
```text
packages/axon/src/cli/cmd/tui
```

Its private dependency on `packages/axon` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/axon/src/*`.
...
Status: Completed. The private `@axon-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@axon-ai/tui`.
...
- The package has no dependency on `axon`, `@axon-ai/cli`, or
  `@axon-ai/core`.
...
primitives now live in `@axon-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@axon-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Axon globals or process state;
...
Replace process-global Axon state with resolved TUI inputs.
...
`@axon-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/axon` initially.
...
canonical in `@axon-ai/tui`. Configured references resolve through the new
...
adapters rather than Axon domain implementations.

Make the SDK the only Axon domain boundary used by the TUI.
...
- Domain-facing TUI code imports Axon data and operations only from
  `@axon-ai/sdk`.
...
`@axon-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/axon`.
...
- The package has no imports from `packages/axon`, `packages/cli`, or
  `@axon-ai/core`.
...
public `@axon-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/axon`.
- Keep the legacy embedded worker and server startup in `packages/axon`.
- Change those adapters to load config, create transport inputs, and call the
  public `@axon-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/axon`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@axon-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@axon-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Axon domain state.
...
cd packages/axon && bun typecheck
...
rg '@axon-ai/core|packages/axon|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/axon/src packages/axon/test
```
...
- Build the current-platform `packages/axon` binary.

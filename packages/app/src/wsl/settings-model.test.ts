...
import { enterWslAxonStep, wslAxonAction, wslRuntimeRetryable } from "./settings-model"
...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when Axon needs attention", () => {
    expect(wslAxonAction(undefined)).toBeUndefined()
    expect(
      wslAxonAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    expect(
      wslAxonAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/axon",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("probes the selected distro before entering the Axon step", async () => {
    const calls: string[] = []
    await enterWslAxonStep(
      "Debian",
      async (distro) => calls.push(distro),
      (step) => calls.push(step),
    )
    expect(calls).toEqual(["Debian", "axon"])
  })
})

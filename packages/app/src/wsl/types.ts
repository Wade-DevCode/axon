...
export type WslJob =
  ...
  | { kind: "probe-axon"; distro: string; startedAt: number }
  | { kind: "install-axon"; distro: string; startedAt: number }
...
export type WslServersPlatform = {
  ...
  probeAxon(name: string): Promise<void>
  installAxon(name: string): Promise<void>

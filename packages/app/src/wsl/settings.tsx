...
import { Tag } from "@axon-ai/ui/v2/badge-v2"
import { ButtonV2 } from "@axon-ai/ui/v2/button-v2"
import { Dialog } from "@axon-ai/ui/v2/dialog-v2"
...
import { wslAxonAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.axonChecks[item.config.distro]
          const axonAction = () => wslAxonAction(check())
          const busy = () => wsl.data?.job?.kind === "install-axon" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={axonAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installAxon(item.config.distro))}
                    >

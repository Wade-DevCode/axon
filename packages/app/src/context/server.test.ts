  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://axon.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://axon.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://axon.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          ...
          lastProject: { local: "/local", "https://axon.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://axon.example.com"),
      ),
    ).toEqual({

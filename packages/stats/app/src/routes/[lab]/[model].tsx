...
export default function StatsModel() {
  ...
  const modelTitle = createMemo(() => `${modelName()} Usage, Cost & Rank | Axon Data`)
  const modelDescription = createMemo(
    () =>
      `View ${modelName()} Axon Go usage data, including token volume, weekly rank, token mix, costs, cache ratio, sessions, geo breakdowns, and peer models.`,
  )
      ...
      <div data-slot="model-hero-grid">
        <div data-slot="model-hero-copy">
          ...
          <Show
            when={props.data}
            fallback={
              <p>Model facts from the shared model index. Axon Go usage appears once this model has activity.</p>
            }
          >
            {(data) => (
              <p>
                {data().rank === null
                  ? "Unranked across last week's Axon Go usage"
                  : `Ranked #${data().rank} across last week's Axon Go usage`}{" "}
...
function ModelCatalogCallout(props: { catalog: ModelCatalogEntry | null }) {
  return (
    <div data-component="model-rank-panel">
      ...
      <p>No Axon Go usage in the current data window.</p>
    </div>
  )
}
...
function ModelUsageSection(props: { data: ModelUsagePoint[] }) {
  return (
    <section id="usage" data-section="model-panel">
      <SectionTitle title="Usage" description="Daily Axon Go token volume over the recent two-month window." />
      <Show
...
function ModelEfficiencySection(props: { data: StatsModelData | null; catalog: ModelCatalogEntry | null }) {
  return (
    <section id="efficiency" data-section="model-panel">
      <SectionTitle title="Efficiency" description="Cost, cache behavior, and average Axon Go session shape." />
      <Show
        when={props.data}
        fallback={
...
function ModelPeersSection(props: { data: StatsModelData | null }) {
  return (
    <section id="peers" data-section="model-panel">
      <SectionTitle title="Peers" description="Nearby models by recent Axon Go token volume." />
      <Show

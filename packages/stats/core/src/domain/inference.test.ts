...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve axon route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "axon")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "axon")).toBe("openai")
    expect(statProvider("big-pickle", "", "axon")).toBe("unknown")
  ...
  test("provider aggregates never keep axon as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "axon"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "axon"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep axon or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "axon"), country: "US" })).toMatchObject([

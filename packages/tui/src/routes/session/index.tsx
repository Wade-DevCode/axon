...
import { AXON_BASE_MODE, useBindings, useCommandShortcut, useAxonKeymap } from "../../keymap"
...
import { changeSummary, sessionActivity, sessionTimeline, statusReportIndex, type TimelineEntry } from "./presentation"
...
const GO_UPSELL_PROVIDERS = new Set(["axon", "axon-go"])
...
export function Session() {
  ...
  const statusIndex = createMemo(() => {
    if (local.status.sessionID() !== route.sessionID) return -1
    return statusReportIndex(messages(), local.status.afterMessageID())
  })
          ...
          >
            <Show when={session()}>
              ...
              >
                ...
                <Show when={statusIndex() === 0}>
                  <StatusReport sessionID={route.sessionID} />
                </Show>
                <For each={messages()}>
                  {(message, index) => (
                    <>
        ...
        <Match when={props.last || final() || props.message.error?.name === "MessageAbortedError"}>
          <box ref={(el: BoxRenderable) => alwaysSeparate.add(el)} paddingLeft={3}>
            <text marginTop={1}>
              ...
              <span style={{ fg: theme.text }}>{activity()}</span>

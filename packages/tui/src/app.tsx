...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "session") {
      ...
      if (!session || isDefaultTitle(session.title)) {
        terminalTitle.set("Axon", working && kv.get("animations_enabled", true))
        return
      }

      const title = session.title.length > 40 ? session.title.slice(0, 37) + "..." : session.title
      terminalTitle.set(`Axon | ${title}`, working && kv.get("animations_enabled", true))
      return
    }
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to Axon v${result.data.version}. Please restart the application.`,
    )

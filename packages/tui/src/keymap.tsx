...
export const AXON_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const AXON_MODE_KEY = "axon.mode"

export const AxonKeymapProvider = KeymapProvider
export const useAxonKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type AxonModeStack = ReturnType<typeof createAxonModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, AxonModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createAxonModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(AXON_MODE_KEY, AXON_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(AXON_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(AXON_MODE_KEY, stack.at(-1)?.mode ?? AXON_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? AXON_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(AXON_MODE_KEY, undefined)
...
export function useAxonModeStack() {
  return getAxonModeStack(useAxonKeymap())
}

export function getAxonModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Axon mode stack is not registered for this keymap")
  return value
}
...
export function registerAxonKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createAxonModeStack(keymap)

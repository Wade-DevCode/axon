import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@axon-ai/plugin/tui"
...
import type { useAxonKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useAxonKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getAxonModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getAxonModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {

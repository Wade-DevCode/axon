...
import { getAxonModeStack, AXON_BASE_MODE, AxonKeymapProvider, registerAxonKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerAxonKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when axon mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getAxonModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getAxonModeStack(keymap).push("autocomplete")

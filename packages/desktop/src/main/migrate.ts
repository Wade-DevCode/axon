...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.axon.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `axon.settings.dat` is special: it maps to the `axon.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"axon.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // axon.settings.dat → the electron settings store ("axon.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "axon.global.dat").
  const storeName = filename === "axon.settings.dat" ? "axon.settings" : filename

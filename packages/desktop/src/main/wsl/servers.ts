...
type WslServersControllerOptions = {
  ...
  resolveAxon?: typeof resolveWslAxon
...
export function createWslServersController(
  ...
  const setAxonCheck = (distro: string, check: WslAxonCheck) => {
    setState({
      axonChecks: {
        ...state.axonChecks,
        [distro]: check,
  ...
  const checkAxon = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveAxon ?? resolveWslAxon)(distro, opts)
  ...
  const refreshAxonCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setAxonCheck(distro, await checkAxon(distro, opts))
  }
  ...
  const refreshAxonCheckBackground = (id: string, distro: string) => {
    void checkAxon(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setAxonCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl axon check failed", { id, distro, message })
      })
  }

  const refreshAxonChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkAxon(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setAxonCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl axon check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshAxonCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async probeAxon(name: string) {
      await runJob({ kind: "probe-axon", distro: name, startedAt: Date.now() }, async (abort) => {
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.axonChecks, distro) : {}),
      })
    },

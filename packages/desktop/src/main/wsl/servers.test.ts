...
let releaseAxonResolve: (() => void) | undefined
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.axon/bin/axon",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, axonChecks: {} })
})
...
test("ignores stale background Axon checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  await waitFor(() => !!releaseAxonResolve)
  await controller.removeServer("wsl:Debian")
  releaseAxonResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().axonChecks).toEqual({})
})

test("ignores stale startup Axon checks after removing a WSL server", async () => {
  ...
  return {
    ...
    resolveAxon: async () => {
      await new Promise<void>((resolve) => {
        releaseAxonResolve = resolve
      })
      return "/home/me/.axon/bin/axon"
    },
  }
}

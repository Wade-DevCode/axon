...
import { type WslCommandLine, resolveWslAxon, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const axon = await resolveWslAxon(distro)
  if (!axon) throw new Error(`Axon is not installed in ${distro}`)
  ...
  const script = [
    ...
    "export PATH",
    "export WSLENV=",
    "export AXON_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export AXON_CLIENT=desktop",
    `export AXON_SERVER_USERNAME=${shellEscape(username)}`,
    `export AXON_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(axon)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")

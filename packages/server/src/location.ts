...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-axon-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-axon-directory"] ? decode(request.headers["x-axon-directory"]) : process.cwd())

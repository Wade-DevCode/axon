---
title: Server
description: Interact with axon server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `axon serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an axon client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `axon.local` |
...
Set `AXON_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `axon`, or set `AXON_SERVER_USERNAME` to override it. This applies to both `axon serve` and `axon web`.
...
When you run `axon` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the axon server to interact with axon programmatically.
:::

This architecture lets axon support multiple clients and allows you to interact with axon programmatically.

You can run `axon serve` to start a standalone server. If you have the
axon TUI running, `axon serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Axon [IDE](/docs/ide) plugins.
...
The axon server exposes the following APIs.

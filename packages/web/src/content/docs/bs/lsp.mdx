---
title: LSP serveri
description: Axon se integriše sa vašim LSP serverima.
---

Axon se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

Axon dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `AXON_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i axon otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj axon konfiguraciji.

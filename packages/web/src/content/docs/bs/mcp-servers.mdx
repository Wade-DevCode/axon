...
Možete dodati vanjske alate u Axon koristeći _Model Context Protocol_, ili MCP. Axon podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Axon Config](https://opencode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/axon`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Axon automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Axon će:
...
Ako server zahtijeva autentifikaciju, Axon će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `axon mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Axon sigurno čuva tokene u `~/.local/share/axon/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Axon, zajedno s ugrađenim alatima. Možete njima upravljati kroz Axon konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Axon sa vašim Sentry nalogom.

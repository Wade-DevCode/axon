...
U Axon mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Axon koristi nasu `axon` temu.
...
Axon dolazi sa vise ugradenih tema.
...
- Zele da Axon odgovara izgledu njihovog terminala
...
Axon podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.axon/themes/*.json` ili `$XDG_CONFIG_HOME/axon/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.axon/themes/*.json`
4. **Trenutni radni direktorij** - `./.axon/themes/*.json`

...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Axon dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Axon.
...
:::note
Ovaj alat je dostupan samo kada je `AXON_EXPERIMENTAL_LSP_TOOL=true` (ili `AXON_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Axon provajdera ili kada je varijabla `AXON_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Axon:

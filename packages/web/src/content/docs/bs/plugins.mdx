---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Axon.
---

Dodaci vam omogućavaju da proširite Axon spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Axon.
...
- `.axon/plugins/` - Dodaci na nivou projekta
- `~/.axon/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/axon/node_modules/`.
...
1. Globalna konfiguracija (`~/.axon/axon.json`)
2. Konfiguracija projekta (`axon.json`)
3. Globalni direktorij dodataka (`~/.axon/plugins/`)
4. Direktorij dodataka projekta (`.axon/plugins/`)
...
Axon pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Axon SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Axon.
...
Spriječite axon da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u axon:
...
Pomoćnik `tool` kreira prilagođeni alat koji axon može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za axon zajedno sa ugrađenim alatima.

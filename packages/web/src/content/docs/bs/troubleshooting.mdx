...
Da biste otklonili probleme s Axon, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\axon\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `axon --log-level DEBUG`.

---

## Pohrana

Axon pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/axon/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\axon`
...
Axon Desktop pokreće lokalni Axon server (`axon-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Axon` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.axon/axon.jsonc` (ili `~/.axon/axon.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/axon/axon.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\axon\axon.jsonc`
...
Axon također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.axon/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\axon\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Axon mogao ponovo izgraditi.

1. Potpuno zatvorite Axon Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/axon`
- **Linux**: obrišite `~/.cache/axon` (ili pokrenite `rm -rf ~/.cache/axon`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\axon`

3. Ponovo pokrenite Axon Desktop.

---

### Rješavanje problema sa vezom na serveru

Axon Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `axon.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `AXON_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `AXON_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Axon Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Axon karakteristikama.

---

### Obavještenja se ne prikazuju

Axon Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Axon u postavkama vašeg OS-a, i
...
1. Zatvorite Axon Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Axon Desktop aplikacije):

- `axon.settings.dat` (URL zadanog servera za desktop)
- `axon.global.dat` i `axon.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `axon upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `axon models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\axon`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Axon dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\axon`

2. Ponovo pokrenite Axon da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Axon da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Axon će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.

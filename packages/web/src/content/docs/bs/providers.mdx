---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Axon.
---

import config from "../../../../config.mjs"
export const console = config.console

Axon koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Axon konfiguraciji.
...
u `~/.local/share/axon/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Axon
config.
...
Axon Zen je lista modela koje je obezbedio Axon tim koji su bili
testirano i potvrđeno da dobro radi sa Axon. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Axon Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite axon i idite na [opencode.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u Axon i potpuno je opcionalan za korištenje.

---

## Axon Go

Axon Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Axon tim i koji su testirani i verificirani da dobro rade s Axon-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Axon Go` i idite na [opencode.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s Axon:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete axon:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `axon.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Axon.
:::
...
Možete konfigurirati axon za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi axon ispravno radio.
   :::
...
Također možete dodati modele kroz svoju axon konfiguraciju.
...
da pohrani token u axon auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Axon koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Axon
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`axon.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Da biste koristili svoju GitHub Copilot pretplatu s axon:
...
Za korištenje Google Vertex AI s Axon:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće axon.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem axona, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [axon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Axon razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Axon razgovor naveden kao zasebna sesija.
...
Možete konfigurirati axon za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati axon za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati axon za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Axon. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Axon:
...
3. Kopirajte API ključ za korištenje u Axon.
...
6. **Važno**: Prije upotrebe modela oblaka u Axon, morate lokalno povući informacije o modelu:
...
Axon Zen je lista testiranih i verifikovanih modela koju je obezbedio Axon tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Axon Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Axon Zen**.

```txt
   /connect
```

3. Unesite svoj Axon API ključ.
...
Također možete dodati dodatne modele putem vaše axon konfiguracije.
...
5. Također ih možete prilagoditi putem vaše axon konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Axonom:
...
Također možete prilagoditi modele kroz svoju axon konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s axon-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `axon.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Axon da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `axon auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Axon konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj axon konfiguraciji.

---
title: Server
description: Komunicirajte s axon serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `axon serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji axon klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `axon.local` |
...
Postavite `AXON_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `axon`, ili postavite `AXON_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `axon serve` i za `axon web`.
...
Kada pokrenete `axon`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite axon server za programsku interakciju sa axon-om.
:::

Ova arhitektura omogucava axon podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `axon serve` da startate standalone server. Ako je axon TUI vec pokrenut, `axon serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Axon [IDE](/docs/ide) pluginovi.
...
axon server izlaže sljedece API-je.

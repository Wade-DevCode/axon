---
title: Konfiguracija
description: Korištenje Axon JSON konfiguracije.
---

Možete konfigurirati Axon koristeći JSON konfiguracijski fajl.

---

## Format

Axon podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/axon`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.axon/axon.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`AXON_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`axon.json` u projektu) - postavke specifične za projekat
5. **`.axon` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`AXON_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.axon` i `~/.axon` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/axon` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Axon konfiguraciju u `~/.axon/axon.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.axon/tui.json`.
...
Dodajte `axon.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Axon pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `AXON_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `AXON_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.axon` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.axon`, tako da **može nadjačati** njihove postavke.
...
Koristite `AXON_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `axon.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `axon serve` i `axon web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Axon server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `axon.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Axon konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Axon pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.axon/agents/` ili `.axon/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Axon će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`axon run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.axon/commands/` ili `.axon/commands/`. [Saznajte više ovdje](/docs/commands).
...
Axon će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Axon **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Axon sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.axon/plugins/` ili `~/.axon/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Axon da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.

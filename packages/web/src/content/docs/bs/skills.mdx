...
Agent skills omogucavaju Axon da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Axon pretrazuje ove lokacije:

- Konfiguracija projekta: `.axon/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.axon/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Axon ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.axon/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.axon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.axon/skills/git-release/SKILL.md` ovako:
...
Axon navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `axon.json`:
...
**Za ugradene agente** (u `axon.json`):

---
title: Serveur
description: Interagissez avec le serveur axon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `axon serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client axon peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `axon.local` |
...
Définissez `AXON_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `axon`, ou définissez `AXON_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `axon serve` et à `axon web`.
...
Lorsque vous exécutez `axon`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur axon pour interagir avec axon par programmation.
:::

Cette architecture permet à axon de prendre en charge plusieurs clients et vous permet d'interagir avec axon par programme.

Vous pouvez exécuter `axon serve` pour démarrer un serveur autonome. Si vous avez le
axon TUI en cours d'exécution, `axon serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Axon [IDE](/docs/ide).
...
Le serveur axon expose les API suivantes.

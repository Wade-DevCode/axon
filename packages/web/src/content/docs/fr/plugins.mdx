---
title: Plugins
description: Écrivez vos propres plugins pour étendre Axon.
---

Les plugins vous permettent d'étendre Axon en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Axon.
...
- `.axon/plugins/` - Plugins au niveau du projet
- `~/.axon/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/axon/node_modules/`.
...
1. Configuration globale (`~/.axon/axon.json`)
2. Configuration du projet (`axon.json`)
3. Répertoire global des plugins (`~/.axon/plugins/`)
4. Répertoire des plugins du projet (`.axon/plugins/`)
...
Axon exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK axon pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre axon.
...
:::note
Si vous utilisez l'application de bureau Axon, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez axon de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à axon :
...
L'assistant `tool` crée un outil personnalisé que axon peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour axon aux côtés des outils intégrés.

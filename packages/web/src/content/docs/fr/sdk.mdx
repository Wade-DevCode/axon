---
title: SDK
description: Client JS de type sécurisé pour le serveur axon.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK axon JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler axon par programme.
...
Créez une instance de axon :

```javascript
import { createAxon } from "@axon-ai/sdk"

const { client } = await createAxon()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `axon.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${axon.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de axon, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@axon-ai/sdk"
```

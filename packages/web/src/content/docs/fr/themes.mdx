...
Avec Axon, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Axon utilise notre propre thème `axon`.
...
Axon est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Axon corresponde à l'apparence de leur terminal
...
Axon prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.axon/themes/*.json` ou `$XDG_CONFIG_HOME/axon/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.axon/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.axon/themes/*.json`

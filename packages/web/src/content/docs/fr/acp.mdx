---
title: Support ACP
description: Utilisez Axon dans n’importe quel éditeur compatible ACP.
---

Axon prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser Axon via ACP, configurez votre éditeur pour exécuter la commande `axon acp`.

La commande démarre Axon en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "Axon": {
      "command": "/absolute/path/bin/axon",
      "args": ["acp"]
    }
  }
}
...
Pour l'ouvrir, utilisez le nouvel agent « Axon » dans le sélecteur d'agent AI Chat.
...
Pour utiliser Axon comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser Axon comme agent ACP pour le chat.

Si vous devez transmettre des variables d'environnement (comme `AXON_API_KEY`), reportez-vous à [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) dans la documentation CodeCompanion.nvim pour plus de détails.

## Prise en charge

Axon fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration Axon

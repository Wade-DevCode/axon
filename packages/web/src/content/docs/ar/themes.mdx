...
مع Axon يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Axon سمتنا `axon`.
...
- يريدون أن يطابق Axon مظهر terminal لديهم
...
يدعم Axon نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.axon/themes/*.json` أو `$XDG_CONFIG_HOME/axon/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.axon/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.axon/themes/*.json`

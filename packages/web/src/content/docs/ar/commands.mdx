...
أنشئ الملف `.axon/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Axon أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Axon:
...
- على مستوى النظام: `~/.axon/commands/`
- لكل مشروع: `.axon/commands/`
...
يتضمن axon عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).

---
title: الخادم
description: تفاعل مع خادم axon عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `axon serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل axon استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `axon.local` |
...
عيّن `AXON_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `axon`، أو عيّن `AXON_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `axon serve` و `axon web`.
...
عند تشغيل `axon` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم axon للتفاعل مع axon برمجيا.
:::

تتيح هذه البنية لـ axon دعم عدة عملاء وتمكّنك من التفاعل مع axon برمجيا.

يمكنك تشغيل `axon serve` لبدء خادم مستقل. إذا كانت واجهة axon في terminal (TUI)
قيد التشغيل، فسيبدأ `axon serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Axon لـ [IDE](/docs/ide).
...
يعرض خادم axon واجهات API التالية.

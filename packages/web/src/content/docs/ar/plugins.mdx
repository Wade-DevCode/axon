---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Axon.
---

تتيح لك الإضافات توسيع Axon عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Axon.
...
- `.axon/plugins/` - إضافات على مستوى المشروع
- `~/.axon/plugins/` - إضافات عامة
...
**إضافات npm** تُثبَّت تلقائيا باستخدام Bun عند بدء التشغيل. تُخزَّن الحزم واعتمادياتها مؤقتا في `~/.cache/axon/node_modules/`.
...
1. إعدادات عامة (`~/.axon/axon.json`)
2. إعدادات المشروع (`axon.json`)
3. دليل الإضافات العام (`~/.axon/plugins/`)
4. دليل إضافات المشروع (`.axon/plugins/`)
...
يشغّل Axon الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ axon للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع axon.
...
:::note
إذا كنت تستخدم تطبيق Axon لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى axon:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ axon استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ axon إلى جانب الأدوات المضمنة.

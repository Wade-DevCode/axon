---
title: Supporto ACP
description: Usa Axon in qualunque editor compatibile con ACP.
---

Axon supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Axon via ACP, configura l'editor in modo che esegua il comando `axon acp`.

Il comando avvia Axon come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'Axon' nel selettore agenti di AI Chat.
...
Per usare Axon come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Axon come agente ACP per la chat.

Se devi passare variabili d'ambiente (come `AXON_API_KEY`), vedi [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) nella documentazione di CodeCompanion.nvim.

## Supporto

Axon funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione Axon

...
Le skill degli agenti permettono a Axon di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Axon cerca in queste posizioni:

- Config di progetto: `.axon/skills/<name>/SKILL.md`
- Config globale: `~/.axon/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Axon risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.axon/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.axon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.axon/skills/git-release/SKILL.md` cosi':
...
Axon elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `axon.json`:
...
**Per agenti integrati** (in `axon.json`):

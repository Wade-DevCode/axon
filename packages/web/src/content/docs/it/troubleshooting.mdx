...
Per diagnosticare problemi con Axon, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\axon\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `axon --log-level DEBUG`.

---

## Archiviazione

axon salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/axon/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\axon`
...
Axon Desktop esegue in background un server locale di Axon (il sidecar `axon-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Axon` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/axon/axon.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\axon\axon.jsonc`
...
Axon puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.axon/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\axon\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Axon possa ricostruirla.

1. Chiudi completamente Axon Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/axon`
- **Linux**: elimina `~/.cache/axon` (oppure esegui `rm -rf ~/.cache/axon`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\axon`

3. Riavvia Axon Desktop.

---

### Risolvi problemi di connessione al server

Axon Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `axon.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `AXON_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `AXON_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Axon Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Axon.

---

### Notifiche non visualizzate

Axon Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Axon nelle impostazioni del sistema operativo, e
...
1. Chiudi Axon Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Axon Desktop):

- `axon.settings.dat` (desktop default server URL)
- `axon.global.dat` e `axon.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Axon:
...
3. Assicurati di avere l'ultima versione con `axon upgrade`
...
Per capire a quali modelli hai accesso, esegui `axon models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\axon`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. axon installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\axon`

2. Riavvia axon per reinstallare i pacchetti provider piu recenti

Questo forzera axon a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
axon rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.

---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Axon.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Axon fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Axon avvia la TUI per la directory corrente.
...
Quando usi la TUI di Axon, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Axon. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Questo è separato da `axon.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Axon attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `axon.default`.

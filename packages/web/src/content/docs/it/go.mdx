...
Axon Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Axon. Ti abboni a Axon Go e
...
Axon Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Axon Go funziona come qualsiasi altro provider in Axon.

1. Accedi a **<a href={console}>Axon Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Axon Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Axon Go.
:::
...
Axon Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Axon config
utilizza il formato `axon-go/<model-id>`. Ad esempio, per Kimi K2.7 Code, useresti
`axon-go/kimi-k2.7-code` nella tua configurazione.
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Axon.

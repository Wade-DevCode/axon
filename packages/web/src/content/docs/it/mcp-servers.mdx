...
Puoi aggiungere strumenti esterni a Axon usando il _Model Context Protocol_, o MCP. Axon supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Axon Config](https://opencode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/axon`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Axon gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Axon:
...
Se il server richiede autenticazione, Axon ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `axon mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Axon memorizzera' i token in modo sicuro in `~/.local/share/axon/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Axon insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Axon come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Axon al tuo account Sentry.

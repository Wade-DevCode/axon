---
title: Server
description: Interagisci con il server axon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `axon serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client axon.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `axon.local` |
...
Imposta `AXON_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `axon`, oppure imposta `AXON_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `axon serve` sia per `axon web`.
...
Quando esegui `axon` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server axon per interagire con axon in modo programmatico.
:::

Questa architettura permette a axon di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `axon serve` per avviare un server standalone. Se la TUI di axon e gia in esecuzione, `axon serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Axon.
...
Il server axon espone le seguenti API.

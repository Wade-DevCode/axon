...
description: Usa la configurazione JSON di Axon.
---

Puoi configurare Axon usando un file di configurazione JSON.

---

## Formato

Axon supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/axon`) - default dell'organizzazione
2. **Config globale** (`~/.axon/axon.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `AXON_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`axon.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.axon`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `AXON_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.axon` e `~/.axon` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/axon`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di Axon in `~/.axon/axon.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.axon/tui.json`.
...
Aggiungi `axon.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando Axon si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `AXON_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `AXON_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.axon` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.axon`, quindi **puo sovrascrivere** le loro impostazioni.
...
Usa `AXON_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `axon.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `axon serve` e `axon web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Axon.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `axon.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in Axon tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Axon prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.axon/agents/` o `.axon/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Axon fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`axon run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.axon/commands/` o `.axon/commands/`. [Scopri di piu](/docs/commands).
...
Axon scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, axon **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Axon con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.axon/plugins/` o `~/.axon/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Axon a usare solo alcuni provider invece di disabilitarli uno per uno.

---
title: Plugin
description: Scrivi plugin per estendere Axon.
---

I plugin ti permettono di estendere Axon agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Axon.
...
- `.axon/plugins/` - plugin a livello progetto
- `~/.axon/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/axon/node_modules/`.
...
1. Config globale (`~/.axon/axon.json`)
2. Config di progetto (`axon.json`)
3. Directory plugin globale (`~/.axon/plugins/`)
4. Directory plugin di progetto (`.axon/plugins/`)
...
Axon esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di axon per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere axon.
...
:::note
Se usi l'app desktop di Axon, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a axon di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a axon:
...
L'helper `tool` crea uno strumento personalizzato che axon può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in axon insieme agli strumenti integrati.

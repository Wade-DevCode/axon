...
Com o axon, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o axon usa nosso próprio tema `axon`.
...
O axon vem com vários temas embutidos.
...
- Querem que o axon corresponda à aparência do seu terminal
...
O axon suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.axon/themes/*.json` ou `$XDG_CONFIG_HOME/axon/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.axon/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.axon/themes/*.json`

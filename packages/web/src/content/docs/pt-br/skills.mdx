...
As habilidades do agente permitem que o axon descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O axon pesquisa nesses locais:

- Configuração do projeto: `.axon/skills/<name>/SKILL.md`
- Configuração global: `~/.axon/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o axon sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.axon/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.axon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.axon/skills/git-release/SKILL.md` assim:
...
O axon lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `axon.json`:
...
**Para agentes embutidos** (em `axon.json`):

---
title: Servidor
description: Interaja com o servidor axon via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `axon serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente axon pode usar.
...
```bash
axon serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `axon.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`         |

`--cors` pode ser passado várias vezes:

```bash
axon serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `AXON_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `axon`, ou defina `AXON_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `axon serve` quanto ao `axon web`.

```bash
AXON_SERVER_PASSWORD=your-password axon serve
```
...
Quando você executa `axon`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor axon para interagir com o axon programaticamente.
:::

Essa arquitetura permite que o axon suporte múltiplos clientes e permite que você interaja com o axon programaticamente.

Você pode executar `axon serve` para iniciar um servidor autônomo. Se você tiver o TUI do axon em execução, `axon serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do axon [IDE](/docs/ide).
...
O servidor axon expõe as seguintes APIs.

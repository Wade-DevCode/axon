---
title: Plugins
description: Escreva seus próprios plugins para estender o axon.
---

Plugins permitem que você estenda o axon conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do axon.
...
- `.axon/plugins/` - Plugins em nível de projeto
- `~/.axon/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/axon/node_modules/`.
...
1. Configuração global (`~/.axon/axon.json`)
2. Configuração do projeto (`axon.json`)
3. Diretório de plugins global (`~/.axon/plugins/`)
4. Diretório de plugins do projeto (`.axon/plugins/`)
...
O axon executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do axon para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o axon.
...
:::note
Se você estiver usando o aplicativo desktop axon, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o axon de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao axon:
...
O helper `tool` cria uma ferramenta personalizada que o axon pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o axon junto com as ferramentas integradas.

---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć axon.
---

Wtyczki umożliwiające wyłączenie axon poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania axon.
...
- `.axon/plugins/` - Wtyczki na poziomie projektu
- `~/.axon/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/axon/node_modules/`.
...
1. Konfiguracja globalna (`~/.axon/axon.json`)
2. Konfiguracja projektu (`axon.json`)
3. Globalny katalog wtyczek (`~/.axon/plugins/`)
4. Katalog wtyczek projektu (`.axon/plugins/`)
...
axon uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z axon do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do axon.
...
:::note
Jeśli korzystasz z aplikacji komputerowej axon, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj axon czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do axon:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod axon. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:

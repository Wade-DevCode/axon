...
Możesz dodać dodatkowe informacje do axon za pomocą _Model Context Protocol_, w skrócie MCP. axon obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [axon Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/axon`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
axon automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, axon:
...
Jeśli serwer wymaga uwierzytelnienia, axon poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `axon mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji axon bezpieczne przechowa tokeny w `~/.local/share/axon/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w axon, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie axon, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć axon z kontem Sentry.

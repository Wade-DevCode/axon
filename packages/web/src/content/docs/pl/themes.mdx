...
Dzięki axon możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie axon używa naszego własnego motywu `axon`.
...
axon ma kilka wbudowanych motywów.
...
- Chcą, aby Axon pasował do wyglądu ich terminala
...
axon obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.axon/themes/*.json` lub `$XDG_CONFIG_HOME/axon/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.axon/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.axon/themes/*.json`

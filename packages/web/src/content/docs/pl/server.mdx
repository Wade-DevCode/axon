---
title: Serwer
description: Interakcja z serwerem axon za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `axon serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient axon.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `axon.local` |
...
Ustaw `AXON_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `axon` lub ustaw `AXON_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `axon serve`, jak i `axon web`.
...
Kiedy uruchomisz `axon`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera axon do programowej interakcji z axon.
:::

Ta architektura umożliwia obsługę wielu klientów przez axon i programową interakcję z kodem otwartym.

Możesz uruchomić `axon serve`, aby uruchomić samodzielny serwer. Jeśli masz
axon TUI działa, `axon serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki axon [IDE](/docs/ide).
...
Serwer axon udostępnia następujące interfejsy API.

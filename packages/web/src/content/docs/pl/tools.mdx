...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Axon posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Axon.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `AXON_EXPERIMENTAL_LSP_TOOL=true` (lub `AXON_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Axon lub gdy zmienna środowiskowa `AXON_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Axon:

...
Umiejętności agenta pozwalają axon odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
axon przeszukuje te lokalizacje:

- Project config: `.axon/skills/<name>/SKILL.md`
- Global config: `~/.axon/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu axon przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.axon/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.axon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.axon/skills/git-release/SKILL.md` w ten sposób:
...
axon wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `axon.json`:
...
**Dla agentów wbudowanych** (w `axon.json`):

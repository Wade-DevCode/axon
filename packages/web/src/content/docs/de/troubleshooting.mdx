...
Wenn Axon Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\axon\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `axon --log-level DEBUG`.

---

## Speicher

axon speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/axon/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\axon`
...
Axon Desktop startet im Hintergrund einen lokalen Axon-Server (`axon-cli`-Sidecar).
...
- Nur macOS: `Axon`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.axon/axon.jsonc` (oder `~/.axon/axon.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/axon/axon.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\axon\axon.jsonc`
...
Axon kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.axon/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\axon\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Axon ihn neu aufbauen kann.

1. Beende Axon Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/axon`
- **Linux**: loesche `~/.cache/axon` (oder fuehre aus: `rm -rf ~/.cache/axon`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\axon`

3. Starte Axon Desktop neu.

---

### Server-Verbindungsprobleme beheben

Axon Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `axon.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `AXON_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `AXON_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Axon Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Axon-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Axon Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Axon in den OS-Einstellungen aktiviert sind, und
...
1. Beende Axon Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Axon Desktop):

- `axon.settings.dat` (desktop default server URL)
- `axon.global.dat` und `axon.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Axon hast:
...
3. Stelle sicher, dass du die neueste Version hast: `axon upgrade`
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `axon models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\axon`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. axon installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\axon`

2. Starte axon neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt axon, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
axon erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).

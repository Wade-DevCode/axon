...
Agent Skills erlauben Axon, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Axon sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Axon vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.axon/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.axon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.axon/skills/git-release/SKILL.md` zum Beispiel so:
...
Axon listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `axon.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `axon.json`):

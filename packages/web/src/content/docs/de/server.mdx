---
title: Server
description: Nutze den axon-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `axon serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein axon-Client nutzen kann.
...
Setze `AXON_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `axon`, kann aber mit `AXON_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `axon serve` und `axon web`.
...
Wenn du `axon` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den axon-Server, um axon programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `axon serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `axon serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Axon-[IDE](/docs/ide)-Plugins.
...
Der axon-Server stellt folgende APIs bereit.

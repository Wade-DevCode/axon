...
description: Verwenden Sie Axon in jedem ACP-kompatiblen Editor.
---

Axon unterstützt [Agent Client Protocol](https://agentclientprotocol.com) oder (ACP), sodass Sie es direkt in kompatiblen Editoren und IDEs verwenden können.
...
Um Axon über ACP zu verwenden, konfigurieren Sie Ihren Editor für die Ausführung des Befehls `axon acp`.

Der Befehl startet Axon als ACP-kompatiblen Unterprozess, der über stdio über JSON-RPC mit Ihrem Editor kommuniziert.
...
Um es zu öffnen, verwenden Sie den neuen Agenten „Axon“ in der AI Chat Agent Selector.
...
Um Axon als ACP-Agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) zu verwenden, fügen Sie Folgendes zu Ihrer Neovim-Konfiguration hinzu:
...
Diese Konfiguration richtet CodeCompanion so ein, dass Axon als ACP-Agent für den Chat verwendet wird.

Wenn Sie Umgebungsvariablen (wie `AXON_API_KEY`) übergeben müssen, finden Sie ausführliche Informationen zu [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) in der Dokumentation zu CodeCompanion.nvim.

## Unterstützung

Axon funktioniert über ACP genauso wie im Terminal. Alle Funktionen werden unterstützt:
...
- MCP-Server, die in Ihrer Axon-Konfiguration konfiguriert sind

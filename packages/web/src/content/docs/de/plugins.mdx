---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Axon zu erweitern.
---

Mit Plugins können Sie Axon erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Axon zu ändern.
...
- `.axon/plugins/` – Plugins auf Projektebene
- `~/.axon/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/axon/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.axon/axon.json`)
2. Projektkonfiguration (`axon.json`)
3. Globales Plugin-Verzeichnis (`~/.axon/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.axon/plugins/`)
...
Axon führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Axon-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Axon erweitern können.
...
:::note
Wenn Sie die Axon-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Axon `.env`-Dateien liest:
...
Plugins können Axon auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Axon aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Axon zur Verfügung.

---
title: Regeln
description: Lege eigene Anweisungen fuer axon fest.
---

Du kannst axon ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in axon den Befehl `/init` aus.
...
So kann axon sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht axon sie zu erweitern.
...
axon kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.axon/AGENTS.md` speichern.
Diese gelten in allen axon-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Axon dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.axon/AGENTS.md` existiert)
...
Beim Start sucht axon in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.axon/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `axon.json` oder in `~/.axon/axon.json` angeben.
...
axon parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit axon.json

Empfohlen ist das `instructions`-Feld in der `axon.json`:
...
Du kannst axon explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `axon.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::

...
Mit Axon kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Axon das `axon`-Theme.
...
Axon liefert mehrere eingebaute Themes mit.
...
- Axon optisch ans Terminal anpassen wollen
...
Axon unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.axon/themes/*.json` oder `$XDG_CONFIG_HOME/axon/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.axon/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.axon/themes/*.json`

...
For at fejlfinde problemer med Axon, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\axon\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `axon --log-level DEBUG`.

---

## Lagring

axon gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/axon/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\axon`
...
Axon Desktop kører en lokal Axon-server (`axon-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Axon`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.axon/axon.jsonc` (eller `~/.axon/axon.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/axon/axon.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\axon\axon.jsonc`
...
Axon kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.axon/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\axon\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Axon kan genopbygge den.

1. Afslut Axon Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/axon`
- **Linux**: slet `~/.cache/axon` (eller kør `rm -rf ~/.cache/axon`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\axon`

3. Start Axon Desktop på ny.

---

### Løs problemer med serverforbindelse

Axon Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `axon.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `AXON_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `AXON_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Axon Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Axon.

---

### Meddelelser vises ikke

Axon Desktop viser kun systemvarsler når:

- varsler er aktiveret for Axon i dine OS-indstillinger, og
...
1. Afslut Axon Desktop.
2. Find og slet disse filer (de findes i Axon Desktop-appens datamappe):

- `axon.settings.dat` (skrivebordsstandardserver URL)
- `axon.global.dat` og `axon.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Axon:
...
For at finde ud af hvilke modeller du har adgang til, kør `axon models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\axon`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. axon installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\axon`

2. Start axon på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge axon til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
axon vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.

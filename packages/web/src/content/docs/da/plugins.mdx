---
title: Plugins
description: Skriv dine egne plugins for at udvide Axon.
---

Plugins giver dig mulighed for at udvide Axon ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Axons standardadfærd.
...
- `.axon/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/axon/node_modules/`.
...
1. Global konfiguration (`~/.axon/axon.json`)
2. Projektkonfiguration (`axon.json`)
3. Global plugin-mappe (`~/.axon/plugins/`)
4. Projekt plugin bibliotek (`.axon/plugins/`)
...
Axon kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En axon SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide axon.
...
:::note
Hvis du bruger Axon desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå axon i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til axon:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som axon kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for axon sammen med indbyggede værktøjer.

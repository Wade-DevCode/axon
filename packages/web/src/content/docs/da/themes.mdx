...
Med Axon kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Axon vårt eget `axon`-tema.
...
Axon kommer med flere innebygde temaer.
...
- Vil at Axon skal matche terminalens utseende
...
Axon støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.axon/themes/*.json` eller `$XDG_CONFIG_HOME/axon/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.axon/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.axon/themes/*.json`

---
title: Server
description: Interager med axon-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `axon serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en axon-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `axon.local` |
...
Indstil `AXON_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `axon`, eller indstil `AXON_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `axon serve` og `axon web`.
...
Når du kører `axon` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug axon-serveren til at interagere med axon programmatisk.
:::

Denne arkitektur lader axon understøtte flere klienter og lader dig interagere med axon programmatisk.

Du kan køre `axon serve` for at starte en selvstændig server. Hvis du har
axon TUI kørende, vil `axon serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Axon [IDE](/docs/ide) plugins.
...
Axon-serveren viser følgende API'er.

...
Her er alle de innebygde verktøyene som er tilgængelige i Axon.
...
:::note
Dette verktøyet er bare tilgængelig når `AXON_EXPERIMENTAL_LSP_TOOL=true` (eller `AXON_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Axon-leverandøren eller når miljøvariabelen `AXON_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).

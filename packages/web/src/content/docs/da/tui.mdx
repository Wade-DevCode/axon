---
title: TUI
description: Brug af Axon-terminalbrugergrænsefladen.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Axon giver en interaktiv terminalgrænseflade eller TUI til at arbejde med dine projekter med en LLM.

At køre Axon starter TUI for den nuværende mappe.
...
Når du bruger Axon TUI, kan du skrive `/` efterfulgt af et kommandonavn for hurtigt at udføre handlinger. For eksempel:
...
Tilføj en udbyder til Axon. Lader dig vælge fra tilgængelige udbydere og tilføje deres API-nøgler.
...
Dette er adskilt fra `axon.json`, som konfigurerer server-/kørselstidsadfærd.
...
- `leader_timeout` - Styrer hvor længe Axon venter efter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID der skal bruges. Standard er `axon.default`.

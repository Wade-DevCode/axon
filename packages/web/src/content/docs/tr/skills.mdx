...
Ajan becerileri, axon'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
axon su konumlari tarar:

- Proje konfigurasyonu: `.axon/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.axon/skills/<name>/SKILL.md`
...
Proje yerel yollarinda axon, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.axon/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.axon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.axon/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
axon, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`axon.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`axon.json` icinde):

...
axon ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak axon kendi `axon` temasını kullanır.
...
axon birden fazla yerleşik temayla gelir.
...
- axon'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
axon, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.axon/themes/*.json` veya `$XDG_CONFIG_HOME/axon/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.axon/themes/*.json`
4. **Geçerli çalışma dizini** - `./.axon/themes/*.json`

...
description: Axon, LSP sunucularınızla bütünleşir.
---

Axon, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Axon, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`AXON_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve axon bir dosyayı açtığında:
...
LSP sunucularını, axon yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.

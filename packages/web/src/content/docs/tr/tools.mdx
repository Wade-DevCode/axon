...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. axon yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
axon içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `AXON_EXPERIMENTAL_LSP_TOOL=true` (veya `AXON_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca axon provider kullanılırken veya `AXON_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

axon başlatırken etkinleştirmek için:

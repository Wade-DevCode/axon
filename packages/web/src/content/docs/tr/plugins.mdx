---
title: Eklentiler
description: Axon'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Axon'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Axon'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.axon/plugins/` - Proje düzeyinde eklentiler
- `~/.axon/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/axon/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.axon/axon.json`)
2. Project config (`axon.json`)
3. Global eklenti dizini (`~/.axon/plugins/`)
4. Proje eklenti dizini (`.axon/plugins/`)
...
Axon bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Axon SDK istemcisi.
...
Axon'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Axon masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Axon'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Axon'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Axon'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Axon'a da hazır olacaktır.

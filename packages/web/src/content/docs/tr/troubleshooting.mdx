...
axon ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\axon\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `axon --log-level DEBUG`.

---

## Depolama

axon, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/axon/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\axon` yapistirin
...
axon Desktop arka planda yerel bir axon sunucusu (`axon-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `axon` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.axon/axon.jsonc` (veya `~/.axon/axon.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/axon/axon.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\axon\axon.jsonc` yapistirin
...
axon diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.axon/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\axon\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), axon'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. axon Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/axon`
- **Linux**: `~/.cache/axon` dizinini silin (veya `rm -rf ~/.cache/axon` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\axon` yapistirin

3. axon Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

axon Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`axon.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `AXON_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `AXON_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta axon Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, axon ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

axon Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda axon icin bildirimler etkinse
...
1. axon Desktop'u kapatin
2. Su dosyalari bulun ve silin (axon Desktop uygulama veri dizininde yer alirlar):

- `axon.settings.dat` (desktop varsayilan sunucu URL'si)
- `axon.global.dat` ve `axon.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
axon ile ilgili bir sorun yasiyorsaniz:
...
3. `axon upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `axon models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\axon`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. axon, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\axon`

2. En guncel provider paketlerini yeniden kurmak icin axon'u yeniden baslatin

Bu, axon'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
axon, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.

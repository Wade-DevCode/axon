...
_Model Bağlam Protokolü_ veya MCP kullanarak axon'a harici araçlar ekleyebilirsiniz. axon hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [axon Config](https://opencode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/axon` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
axon, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde axon şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa axon, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `axon mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından axon, belirteçleri `~/.local/share/axon/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra axon'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi axon yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve axon'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.

...
Axon Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Axon'daki diğer sağlayıcılar gibi çalışır. Axon Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Axon'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Axon Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Axon Go, Axon'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Axon Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Axon Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Axon Go'ya abone olabilir.
:::
...
Axon Go aşağıdaki limitleri içerir:
...
Axon yapılandırmanızdaki [model id](/docs/config/#models) formatı `axon-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `axon-go/kimi-k2.7-code` kullanmalısınız.
...
Axon Go'yu şu amaçlarla oluşturduk:
...
4. Axon ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.

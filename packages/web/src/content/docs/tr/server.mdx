---
title: Sunucu
description: axon sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`axon serve` komutu, axon istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `axon.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `AXON_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `axon` değeridir; değiştirmek isterseniz `AXON_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `axon serve` hem de `axon web` için geçerlidir.
...
`axon` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
axon ile programatik etkileşim için axon sunucusunu kullanın.
:::

Bu mimari, axon'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`axon serve` ile bağımsız bir sunucu başlatabilirsiniz. axon TUI açıksa, `axon serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum axon [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
axon sunucusu aşağıdaki API'leri sunar.

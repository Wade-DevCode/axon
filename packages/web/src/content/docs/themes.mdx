...
With Axon you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Axon uses our own `axon` theme.
...
Axon comes with several built-in themes.
...
- Want Axon to match their terminal's appearance
...
Axon supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.axon/themes/*.json` or `$XDG_CONFIG_HOME/axon/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.axon/themes/*.json`
4. **Current working directory** - Defined in `./.axon/themes/*.json`

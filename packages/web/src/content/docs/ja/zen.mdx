---
title: Axon Zen
description: Axon によって提供されるモデルの厳選されたリスト。
---
...
Axon Zen は、Axon チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は Axon のほかのプロバイダーと同じように動作します。Axon Zen にログインして API キーを取得できます。これは **完全にオプション** であり、Axon を使うために必須ではありません。
...
:::tip
Axon でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
Axon Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

Axon Zen は、Axon のほかのプロバイダーと同じように動作します。

1. **<a href={console}>Axon Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、Axon Zen を選んで API キーを貼り付けます。
...
Axon 設定で使う [model id](/docs/config/#models) は `axon/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `axon/gpt-5.5` を使用します。
...
- DeepSeek V4 Flash Free は期間限定で Axon で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- MiMo-V2.5 Free は期間限定で Axon で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- North Mini Code Free は期間限定で Axon で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Ultra Free は期間限定で Axon で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で Axon で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに Axon では常にほかのプロバイダーも使えるようにすること。

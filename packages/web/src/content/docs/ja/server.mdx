---
title: サーバー
description: HTTP 経由で Axon サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`axon serve` コマンドは、Axon クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `axon.local` |
...
HTTP 基本認証でサーバーを保護するには、`AXON_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `axon` になるか、`AXON_SERVER_USERNAME` を設定してオーバーライドします。これは、`axon serve` と `axon web` の両方に当てはまります。
...
`axon` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Axon サーバーを使用して、プログラムで Axon と対話します。
:::

`axon serve` を実行してスタンドアロンサーバーを起動できます。すでに Axon TUI を実行している場合でも、`axon serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Axon [IDE](/docs/ide) プラグインによって使用されます。

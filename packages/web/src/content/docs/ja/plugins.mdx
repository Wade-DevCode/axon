---
title: プラグイン
description: Axon を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Axon を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Axon のデフォルトの動作を変更したりできます。
...
- `.axon/plugins/` - プロジェクトレベルのプラグイン
- `~/.axon/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/axon/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.axon/axon.json`)
2. プロジェクト設定 (`axon.json`)
3. グローバルプラグインディレクトリ (`~/.axon/plugins/`)
4. プロジェクトプラグインディレクトリ (`.axon/plugins/`)
...
Axon は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Axon SDK クライアント。
...
ここでは、Axon を拡張するために使用できるプラグインの例をいくつか示します。
...
プラグインは Axon にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Axon が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Axon で使用できます。

---
title: TUI
description: Bruke axon-terminalbrukergrensesnittet.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Axon gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre Axon starter TUI for gjeldende katalog.
...
Når du bruker Axon TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til Axon. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Dette er atskilt fra `axon.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge Axon venter etter leader key. Standard er `2000`.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `axon.default`.

---
title: Tjener
description: Samhandle med Axon-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `axon serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Axon-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `axon.local` |
...
Sett `AXON_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `axon` som standard, eller sett `AXON_SERVER_USERNAME` for å overstyre det. Dette gjelder både `axon serve` og `axon web`.
...
Når du kjører `axon` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Axon-serveren til å samhandle med Axon programmatisk.
:::

Denne arkitekturen lar Axon støtte flere klienter og lar deg samhandle med Axon programmatisk.

Du kan kjøre `axon serve` for å starte en frittstående server. Hvis du har
Axon TUI kjører, vil `axon serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Axon [IDE](/docs/ide) plugins.
...
axon-serveren viser følgende APIer.

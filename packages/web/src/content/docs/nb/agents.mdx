...
Det er to typer agenter i Axon; primære agenter og underagenter.
...
Axon kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Axon kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Axon kommer med to innebygde primære agenter og tre innebygde underagenter.
...
En skrivebeskyttet agent for ekstern dokumentasjon og forskning på avhengigheter. Bruk denne når du trenger å klone et avhengighetsrepo inn i Axon sin administrerte cache, inspisere kildekoden til et bibliotek eller kryssjekke lokal kode mot upstream-implementasjoner uten å endre arbeidsområdet ditt.
...
Konfigurer agenter i konfigurasjonsfilen `axon.json`:
...
Hvis ingen temperatur er spesifisert, bruker Axon modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Axon-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Axon-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Axon Zen](/docs/zen), vil du bruke `axon/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `axon models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/Wade-DevCode/axon).
:::

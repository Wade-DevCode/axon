...
Du kan legge til eksterne verktøy i Axon ved å bruke _Model Context Protocol_ eller MCP. Axon støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Axon Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="axon.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/axon`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Axon håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Axon:
...
Hvis serveren krever autentisering, vil Axon be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `axon mcp auth <server-name>`.
...
```bash
axon mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Axon lagre tokens sikkert i `~/.local/share/axon/mcp-auth.json`.
...
axon mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Axon, sammen med innebygde verktøy. Så du kan administrere dem gjennom Axon-konfigurasjonen som alle andre verktøy.
...
```bash
axon mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Axon til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Axon-samtale oppført som en separat økt.

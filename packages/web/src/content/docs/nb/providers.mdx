---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Axon.
---

import config from "../../../../config.mjs"
export const console = config.console

Axon bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Axon-konfigurasjonen.
...
i `~/.local/share/axon/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Axon
konfig.
...
Axon Zen er en liste over modeller levert av Axon-teamet som har vært
testet og verifisert for å fungere godt med Axon. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Axon Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Axon Zen` og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i Axon og er helt valgfritt å bruke.

---

## Axon Go

Axon Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Axon-teamet som har vært
testet og verifisert for å fungere godt med Axon.

1. Kjør kommandoen `/connect` i TUI, velg `Axon Go`, og gå til [opencode.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med Axon:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Axon:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `axon.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i axon støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere axon til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Axon skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Axon-konfigurasjonen.
...
for å lagre token i Axon auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Axon bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Axon
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`axon.json` fil. Det anbefales også å deaktivere øktdeling.
...
Slik bruker du GitHub Copilot-abonnementet med Axon:
...
Slik bruker du Google Vertex AI med Axon:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Axon, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [axon-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver Axon-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Axon-samtale oppført som en separat økt.
...
Du kan konfigurere Axon for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Axon for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Axon for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Axon. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Axon:
...
3. Kopier API-nøkkelen for bruk i Axon.
...
6. **Viktig**: Før du bruker skymodeller i Axon, må du hente modellinformasjonen lokalt:
...
Axon Zen er en liste over testede og verifiserte modeller levert av Axon-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Axon Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Axon Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Axon API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Axon-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Axon-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Axon:
...
Du kan også tilpasse modeller gjennom Axon-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Axon. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `axon.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Axon forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `axon auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Axon-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Axon-konfigurasjonen.

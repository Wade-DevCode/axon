...
description: Skriv dine egne programtillegg for å utvide Axon.
---

Plugins lar deg utvide Axon ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Axon.
...
- `.axon/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/axon/node_modules/`.
...
1. Global konfigurasjon (`~/.axon/axon.json`)
2. Prosjektkonfigurasjon (`axon.json`)
3. Global plugin-katalog (`~/.axon/plugins/`)
4. Prosjektpluginkatalog (`.axon/plugins/`)
...
Axon kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Axon SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Axon.
...
:::note
Hvis du bruker Axon-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Axon fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Axon:
...
`tool`-hjelperen lager et tilpasset verktøy som Axon kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Axon sammen med innebygde verktøy.

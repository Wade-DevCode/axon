...
Las herramientas permiten que LLM realice acciones en su código base. Axon viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Axon.
...
:::note
Esta herramienta solo está disponible cuando `AXON_EXPERIMENTAL_LSP_TOOL=true` (o `AXON_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Axon o cuando la variable de entorno `AXON_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Axon:

```bash
AXON_ENABLE_EXA=1 axon
```

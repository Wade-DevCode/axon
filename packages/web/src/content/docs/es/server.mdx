---
title: Servidor
description: Interactuar con el servidor axon a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `axon serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente axon puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `axon.local`   |
...
Configure `AXON_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `axon`, o configure `AXON_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `axon serve` como a `axon web`.
...
Cuando ejecuta `axon`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor axon para interactuar con axon mediante programación.
:::

Esta arquitectura permite que axon admita múltiples clientes y le permite interactuar con axon mediante programación.

Puede ejecutar `axon serve` para iniciar un servidor independiente. Si tienes el
axon TUI ejecutándose, `axon serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Axon [IDE](/docs/ide).
...
El servidor axon expone las siguientes API.

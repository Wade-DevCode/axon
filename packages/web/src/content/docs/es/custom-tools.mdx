...
description: Cree herramientas que LLM pueda llamar en axon.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de axon como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.axon/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.axon/tools/`.
...
```ts title=".axon/tools/database.ts" {1}
...
```ts title=".axon/tools/project.ts" {8}
import { tool } from "@axon-ai/plugin"
...
```python title=".axon/tools/add.py"
import sys
...
```ts title=".axon/tools/python-add.ts" {10}
import { tool } from "@axon-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".axon/tools/add.py")

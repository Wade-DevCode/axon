...
Las habilidades del agente permiten a Axon descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Axon busca estas ubicaciones:

- Configuración del proyecto: `.axon/skills/<name>/SKILL.md`
- Configuración global: `~/.axon/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Axon sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.axon/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.axon/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.axon/skills/git-release/SKILL.md` así:
...
Axon enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `axon.json`:
...
**Para agentes integrados** (en `axon.json`):

...
Con Axon puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Axon usa nuestro propio tema `axon`.
...
Axon viene con varios temas integrados.
...
- Quiere que Axon coincida con la apariencia de su terminal
...
Axon admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.axon/themes/*.json` o `$XDG_CONFIG_HOME/axon/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.axon/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.axon/themes/*.json`

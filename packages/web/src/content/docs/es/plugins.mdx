---
title: Complementos
description: Escriba sus propios complementos para extender Axon.
---

Los complementos le permiten extender Axon al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Axon.
...
- `.axon/plugins/` - Complementos a nivel de proyecto
- `~/.axon/plugins/` - Complementos globales
...
```json title="axon.json"
{
  ...
  "plugin": ["axon-helicone-session", "axon-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/axon/node_modules/`.
...
1. Configuración global (`~/.axon/axon.json`)
2. Configuración del proyecto (`axon.json`)
3. Directorio global de complementos (`~/.axon/plugins/`)
4. Directorio de complementos del proyecto (`.axon/plugins/`)
...
Axon ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".axon/plugins/my-plugin.ts"
...
```js title=".axon/plugins/example.js"
...
- `client`: Un cliente SDK axon para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar axon.
...
```js title=".axon/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "axon"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Axon, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que axon lea archivos `.env`:

```javascript title=".axon/plugins/env-protection.js"
...
```javascript title=".axon/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a axon:

```ts title=".axon/plugins/custom-tools.ts"
import { type Plugin, tool } from "@axon-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que axon puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para axon junto con las herramientas integradas.
...
```ts title=".axon/plugins/compaction.ts"
import type { Plugin } from "@axon-ai/plugin"
...
```ts title=".axon/plugins/custom-compaction.ts"
import type { Plugin } from "@axon-ai/plugin"

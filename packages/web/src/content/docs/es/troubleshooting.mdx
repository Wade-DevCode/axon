...
Para depurar problemas con Axon, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/axon/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\axon\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `axon --log-level DEBUG`.

---

## Almacenamiento

axon almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/axon/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\axon`
...
Axon Desktop ejecuta un servidor Axon local (el sidecar `axon-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Axon` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.axon/axon.jsonc` (o `~/.axon/axon.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/axon/axon.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\axon\axon.jsonc`
...
Axon también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.axon/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\axon\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.axon/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Axon pueda reconstruirlo.

1. Salga de Axon Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/axon`
- **Linux**: eliminar `~/.cache/axon` (o ejecutar `rm -rf ~/.cache/axon`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\axon`

3. Reinicie el escritorio Axon.

---

### Solucionar problemas de conexión del servidor

Axon Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `axon.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `AXON_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `AXON_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Axon requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Axon.

---

### Notificaciones que no se muestran

Axon Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Axon en la configuración de su sistema operativo, y
...
1. Salga del escritorio Axon.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Axon):

- `axon.settings.dat` (URL del servidor predeterminado de escritorio)
- `axon.global.dat` y `axon.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/Wade-DevCode/axon/issues**](https://github.com/Wade-DevCode/axon/issues)
...
3. Asegúrese de tener la última versión con `axon upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `axon models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/axon
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\axon`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. axon instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\axon`

2. Reinicie axon para reinstalar los paquetes del proveedor más recientes.

Esto obligará a axon a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
axon detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.

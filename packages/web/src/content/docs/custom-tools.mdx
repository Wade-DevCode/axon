---
title: Custom Tools
description: Create tools the LLM can call in axon.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside axon's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.axon/tools/` directory of your project.
- Or globally, by placing them in `~/.axon/tools/`.

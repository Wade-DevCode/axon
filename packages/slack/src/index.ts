...
import { createAxon, type ToolPart } from "@axon-ai/sdk"
...
console.log("🚀 Starting axon server...")
const axon = await createAxon({
  port: 0,
})
console.log("✅ Axon server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await axon.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new axon session...")
    const { client, server } = axon
    ...
    console.log("✅ Created axon session:", createResult.data.id)
  ...
  console.log("📝 Sending to axon:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Axon response:", JSON.stringify(result, null, 2))

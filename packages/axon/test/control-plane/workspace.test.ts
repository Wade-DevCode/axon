...
import { EventSequenceTable } from "@axon-ai/core/event/sql"
...
import { WorkspaceV2 } from "@axon-ai/core/workspace"
import { WorkspaceTable } from "@axon-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  AXON_AUTH_CONTENT: process.env.AXON_AUTH_CONTENT,
  AXON_EXPERIMENTAL_WORKSPACES: process.env.AXON_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.AXON_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@axon.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.AXON_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=axon-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.AXON_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.AXON_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.AXON_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=axon-test")

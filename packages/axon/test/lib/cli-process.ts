// Subprocess test harness for the axon CLI. Spawns the real binary against
...
//
// Configuration flows through axon's built-in test affordances:
//   - AXON_CONFIG_CONTENT      : provider config inline, no files to find
//   - AXON_TEST_HOME           : pins os.homedir() → tmpdir
//   - AXON_DISABLE_PROJECT_CONFIG : skip walking up for axon.json
//   - AXON_PURE                : skip external plugin discovery + install
//   - AXON_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `axon.run` is fully wired. The shape supports adding more
// builders (`axon.serve(opts)`, `axon.acp(opts)`, `axon.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of AxonCli.
import { test, type TestOptions } from "bun:test"
import { FSUtil } from "@axon-ai/core/fs-util"
import { AppProcess } from "@axon-ai/core/process"
...
const axonRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(axonRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    AXON_CONFIG_CONTENT: configJson,
    AXON_DISABLE_PROJECT_CONFIG: "1",
    AXON_PURE: "1",
    AXON_DISABLE_AUTOUPDATE: "1",
    AXON_DISABLE_AUTOCOMPACT: "1",
    AXON_DISABLE_MODELS_FETCH: "1",
    AXON_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `axon run`. New flags should
...
// `axon serve` is a long-lived process — it never exits on its own.
...
// `axon acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type AxonCli = {
  ...
  // Spawn `axon serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `axon acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("axon.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          AXON_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("axon.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("axon.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "axon server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `axon serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("axon.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const axon: AxonCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, axon })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `axon.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "axon") {
...
// every call site. Use this for any test that needs the axon CLI fixture.
//
...
// resources (e.g. `axon.serve`) without an extra `Effect.scoped` wrapper —

...
it.instance("plan agent denies edits except .axon/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".axon/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".axon", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.AXON_TEST_HOME
      process.env.AXON_TEST_HOME = test.directory

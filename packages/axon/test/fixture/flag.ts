...
/**
 * Scoped override for `Flag.AXON_WORKSPACE_ID`. Saves the previous value
...
export function withFixedWorkspaceID(id: WorkspaceV2.ID): Effect.Effect<void, never, Scope.Scope> {
  return Effect.gen(function* () {
    const previous = Flag.AXON_WORKSPACE_ID
    Flag.AXON_WORKSPACE_ID = id
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        Flag.AXON_WORKSPACE_ID = previous
      }),
    )
  })
}

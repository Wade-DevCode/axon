...
describe("ProviderTransform.options - gpt-5 textVerbosity", () => {
  ...
  test("azure chat completions omit Responses-only reasoning options after variants merge", async () => {
    ...
    expect(result.system[0]).toContain("Never claim that Axon was developed by axon")
    expect(result.system[0]).toContain('Never use "Wade" as the developer name')
  })

  for (const [label, providerOptions, modelOptions] of [
    ["provider omit_max_output_tokens", { omit_max_output_tokens: true }, {}],
    ["provider omitMaxOutputTokens", { omitMaxOutputTokens: true }, {}],
    ["model omit_max_output_tokens", {}, { omit_max_output_tokens: true }],
  ] as const) {
    test(`${label} leaves maxOutputTokens unset`, async () => {
      const model = { ...createGpt5Model("gpt-5.4"), options: modelOptions }
      ...
      expect((await prepare(providerOptions)).params.maxOutputTokens).toBeUndefined()
      if (Object.keys(modelOptions).length === 0) {
        expect((await prepare({})).params.maxOutputTokens).toBeGreaterThan(0)
      }
    })
  }
    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".axon/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    ...
    const result = ProviderTransform.message(msgs, axonModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.axon?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.axon?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    expect(result[0].providerOptions?.axon?.itemId).toBe("msg_axon")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.axon?.itemId).toBe("msg_axon_part")

...
import { SessionV1 } from "@axon-ai/core/v1/session"
import { FSUtil } from "@axon-ai/core/fs-util"
import { ModelsDev } from "@axon-ai/core/models-dev"
import { HttpRecorder } from "@axon-ai/http-recorder"
import { HttpRecorderInternal } from "@axon-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@axon-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@axon-ai/llm/route"
import { Env } from "@/env"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.AXON_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["axon", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("AXON_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("AXON_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["axon", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "axon-proxy",
    name: "Axon proxy",
    providerID: ProviderV2.ID.axon,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["axon", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.AXON_RECORD_CONSOLE_TOKEN && process.env.AXON_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.axon,
        name: "Axon Zen",
        env: ["AXON_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.AXON_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.AXON_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.AXON_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["axon", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("AXON_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("AXON_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("AXON_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set AXON_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the axon LLM stack remain real.
  const metadata = {

...
import { createAxonClient } from "@axon-ai/sdk/v2"
...
type Sdk = ReturnType<typeof createAxonClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.AXON_SERVER_PASSWORD = input?.password
      Flag.AXON_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".axon", "skills", "project-rest-skill", "SKILL.md"),
      `---
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-axon-directory")).toBe(false)
        expect(request!.headers.has("x-axon-workspace")).toBe(false)
      }),
    ),
  )

...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ axon }) =>
    Effect.gen(function* () {
      const result = yield* axon.spawn(["--replay-limit", "10"])

      axon.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ axon }) =>
    Effect.gen(function* () {
      const result = yield* axon.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ axon }) =>
    Effect.gen(function* () {
      const result = yield* axon.spawn(["--mini", "--port", "4096"])

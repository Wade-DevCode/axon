...
`,
  ...
  const pure = process.env.AXON_PURE
  const meta = process.env.AXON_PLUGIN_META_FILE
  process.env.AXON_PURE = "1"
  process.env.AXON_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.AXON_PURE
    } else {
      process.env.AXON_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.AXON_PLUGIN_META_FILE
    } else {
      process.env.AXON_PLUGIN_META_FILE = meta
    }
  }
})

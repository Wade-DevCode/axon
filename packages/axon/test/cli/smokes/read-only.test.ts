...
describe("axon read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // AXON_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ axon }) =>
      Effect.gen(function* () {
        const r = yield* axon.spawn(["mcp", "list"])
        axon.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ axon }) =>
      Effect.gen(function* () {
        const r = yield* axon.spawn(["providers", "list"])
        axon.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ axon }) =>
      Effect.gen(function* () {
        const r = yield* axon.spawn(["models"])
        axon.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ axon }) =>
      Effect.gen(function* () {
        const r = yield* axon.spawn(["agent", "list"])
        axon.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh AXON_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ axon }) =>
      Effect.gen(function* () {
        const r = yield* axon.spawn(["session", "list"])
        axon.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ axon }) =>
      Effect.gen(function* () {
        const r = yield* axon.spawn(["stats"])
        axon.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under AXON_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ axon }) =>
      Effect.gen(function* () {
        const r = yield* axon.spawn(["db", "path"])
        axon.expectExit(r, 0, "db path")

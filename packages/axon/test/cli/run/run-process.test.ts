// Subprocess integration tests for `axon run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `axon.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `AXON_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("axon run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, axon }) =>
      Effect.gen(function* () {
        ...
        const result = yield* axon.run("say hi")
  ...
  cliIt.concurrent(
    ...
    ({ llm, axon }) =>
      Effect.gen(function* () {
        ...
        const result = yield* axon.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, axon }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* axon.run("think", { extraArgs: ["--thinking"] })
        axon.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* axon.run("think again")
        axon.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, axon }) =>
      Effect.gen(function* () {
        ...
        const result = yield* axon.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, axon }) =>
      Effect.gen(function* () {
        ...
        const result = yield* axon.run("say hi", { format: "json" })
        axon.expectExit(result, 0)

        const events = axon.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ axon }) =>
      Effect.gen(function* () {
        const result = yield* axon.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, axon }) =>
      Effect.gen(function* () {
        ...
        const result = yield* axon.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, axon }) =>
      Effect.gen(function* () {
        ...
        const result = yield* axon.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, axon }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* axon.run("request permission", { permission: { bash: "ask" } })
        axon.expectExit(denied, 0)
        ...
        const allowed = yield* axon.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        axon.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* axon.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        axon.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, axon }) =>
      Effect.gen(function* () {
        ...
        const server = yield* axon.serve()

        const result = yield* axon.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, axon }) =>
      Effect.gen(function* () {
        const result = yield* axon.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, axon }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* axon.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()

...
import { AxonClient, type GlobalEvent } from "@axon-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<AxonClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<AxonClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<AxonClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<AxonClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<AxonClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://axon.test"),
...
function sdk(
  input: {
    ...
    subscribe?: AxonClient["event"]["subscribe"]
    globalEvent?: AxonClient["global"]["event"]
    promptAsync?: AxonClient["session"]["promptAsync"]
    status?: AxonClient["session"]["status"]
    messages?: AxonClient["session"]["messages"]
    children?: AxonClient["session"]["children"]
    permissions?: AxonClient["permission"]["list"]
    questions?: AxonClient["question"]["list"]
  } = {},
) {
  const client = new AxonClient()

  const subscribe: AxonClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: AxonClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: AxonClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: AxonClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: AxonClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: AxonClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: AxonClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: AxonClient["question"]["list"] = input.questions ?? (() => ok([]))

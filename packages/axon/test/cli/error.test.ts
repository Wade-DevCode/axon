...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/axon.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/axon.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/axon.jsonc", dir: ".axon", suggestion: "axon" },
        expected:
          'Directory ".axon" in /tmp/axon.jsonc is not valid. Rename the directory to "axon" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        expected: "Configuration is invalid at /tmp/axon.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `axon models` to list available models",
      "Or check your config (axon.json) provider/model names",
    ].join("\n")

...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "axon.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "axon.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "axon.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "axon.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["axon.json", "axon.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "axon.json"),
        path.join(project, "axon.jsonc"),
      ])
    })
  })

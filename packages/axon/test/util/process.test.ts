...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.AXON_TEST ?? "")'), {
      env: {
        AXON_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "AXON_TEST_SHELL"], {
      shell: true,
      env: {
        AXON_TEST_SHELL: "ok",
      },
    ...
    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("AXON_TEST_SHELL=ok")
  })

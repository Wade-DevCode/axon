...
Trim `packages/axon/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/axon/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/axon`.
- `[x]` `bun typecheck` from `packages/axon`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/axon/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/axon`.
- `[x]` `bun dev generate > /tmp/axon-openapi.json` from `packages/axon`.
...
- Edit `packages/axon/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/axon-openapi.json` from `packages/axon`.
...
- Check source schemas in files like `packages/axon/src/session/schema.ts`, `packages/axon/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/axon/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/axon/src/server/routes/instance/httpapi/errors.ts` or add new ones there.

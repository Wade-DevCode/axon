...
Practical reference for a future `packages/server` split after the axon
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/axon`.
...
Until enough shared service code lives outside `packages/axon`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/axon`

It should not import `packages/axon` services while `packages/axon`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on axon-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/axon` implementation details.

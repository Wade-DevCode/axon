...
- Author package entrypoint is `@axon-ai/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/axon-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@axon-ai/plugin/tui`.
- `@axon-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@axon-ai/plugin/tui"
...
- The value is a semver range checked against the running Axon version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.axon` is absent, no check is performed (backward compatible).
...
- `axon plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `axon plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.axon` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.axon`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.axon`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.axon` dir or `AXON_CONFIG_DIR`, Axon installs `@axon-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@axon-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Axon's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Axon registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Axon pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@axon-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"axon"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `axon.default`.
...
- Local plugins persist installed themes under the local `.axon/themes` area near the plugin config source.
...
- `--pure` / `AXON_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.axon/plugins/tui-smoke.tsx`
- Local vim plugin: `.axon/plugins/tui-vim.tsx`
- Local smoke config: `.axon/tui.json`
- Local smoke theme: `.axon/plugins/smoke-theme.json`

...
Delete `packages/axon/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/axon/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/axon/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/axon`
- TUI plugin loader tests in `packages/axon` if runtime plugin API wiring changed

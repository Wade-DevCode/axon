...
import { IdeEvent } from "@axon-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["AXON_CALLER"] === "vscode" || process.env["AXON_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.axon"], {
    nothrow: true,
  })

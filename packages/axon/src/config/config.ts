...
import { httpClient } from "@axon-ai/core/effect/layer-node-platform"
...
import type { ConsoleState } from "@axon-ai/core/v1/config/console-state"
...
import { RemoteAuthError } from "@axon-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@axon-ai/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@axon/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["axon.jsonc", "axon.json", "opencode.jsonc", "opencode.json", "config.json"].map((file) =>
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.AXON_CONFIG && !Flag.AXON_CONFIG_DIR && !Flag.AXON_CONFIG_CONTENT) {
      ...
      // Axon-named global config loads last and wins over legacy config.
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "axon.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "axon.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "AXON_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/axon`
        ...
        if (Flag.AXON_CONFIG) {
          yield* merge(Flag.AXON_CONFIG, yield* loadFile(Flag.AXON_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.AXON_CONFIG })
        }

        if (!Flag.AXON_DISABLE_PROJECT_CONFIG) {
          // Legacy config loads first so Axon-named project config wins.
          for (const name of ["opencode", "axon"]) {
        ...
        if (Flag.AXON_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from AXON_CONFIG_DIR", { path: Flag.AXON_CONFIG_DIR })
        }
        ...
        for (const dir of directories) {
          if (
            ...
            dir === Flag.AXON_CONFIG_DIR
          ) {
            const base = dir.endsWith(".opencode") ? "opencode" : "axon"
          ...
          // Auto-discovered plugins under `.axon/plugin(s)` are already local files, so ConfigPlugin.load
          // returns normalized Specs and we only need to attach origin metadata here.
          const list = yield* Effect.promise(() => ConfigPlugin.load(dir))
          yield* mergePluginOrigins(dir, list)
        }

        if (process.env.AXON_CONFIG_CONTENT) {
          const source = "AXON_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.AXON_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from AXON_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["AXON_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("AXON_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (existsSync(managedDir)) {
          for (const file of ["axon.json", "axon.jsonc"]) {
        ...
        if (Flag.AXON_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.AXON_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("AXON_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.AXON_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.AXON_DISABLE_PRUNE) {

...
import { isRecord } from "@axon-ai/tui/util/record"
...
import { TuiKeybind } from "@axon-ai/tui/config/keybind"
...
import { TuiConfig } from "@axon-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@axon/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old axon.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.axon`
  // folders between cwd and home, and AXON_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.AXON_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit AXON_TUI_CONFIG override, if set.
  if (Flag.AXON_TUI_CONFIG) {
    const configFile = Flag.AXON_TUI_CONFIG
  ...
  // 4. Project config directories (and AXON_CONFIG_DIR) discovered while

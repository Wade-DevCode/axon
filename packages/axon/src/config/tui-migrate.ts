...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from axon.json files
...
async function configFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...
    ...ConfigPaths.fileInDirectory(Global.Path.config, "axon"),
    ...(await Filesystem.findUp(["opencode.json", "opencode.jsonc", "axon.json", "axon.jsonc"], input.cwd, undefined, {
      rootFirst: true,
    })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "opencode"))
    files.push(...ConfigPaths.fileInDirectory(dir, "axon"))
  }
  if (Flag.AXON_CONFIG) files.push(Flag.AXON_CONFIG)

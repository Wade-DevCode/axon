...
const MANAGED_PLIST_DOMAIN = "ai.axon.managed"

// Keys injected by macOS/MDM into the managed plist that are not Axon config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    ...
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "axon")
    default:
      return "/etc/axon"
  }
}
...
export function managedConfigDir() {
  return process.env.AXON_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}

import type { PermissionV1 } from "@axon-ai/core/v1/permission"
import { FSUtil } from "@axon-ai/core/fs-util"
// CLI entry point for `axon run` and `axon --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`axon --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`axon --mini --attach`): connects to a running
//      Axon server and runs interactive mode against it.
//
...
import { createAxonClient, type AxonClient, type ToolPart } from "@axon-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<AxonClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run axon with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running axon server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to AXON_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to AXON_SERVER_USERNAME or 'axon')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: AxonClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: AxonClient, sessionID: string) {
      ...
      async function current(sdk: AxonClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: AxonClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: AxonClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: AxonClient) {
        ...
        async function loop(client: AxonClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createAxonClient({
        baseUrl: "http://axon.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)

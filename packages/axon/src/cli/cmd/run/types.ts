// Shared type vocabulary for the direct interactive mode (`axon --mini`).
//
...
import type { AxonClient, PermissionRequest, QuestionRequest, ToolPart } from "@axon-ai/sdk/v2"
import type { TuiConfig } from "@axon-ai/tui/config"
...
type PromptModel = Parameters<AxonClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<AxonClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<AxonClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<AxonClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<AxonClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<AxonClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<AxonClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<AxonClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<AxonClient["question"]["reject"]>[0]

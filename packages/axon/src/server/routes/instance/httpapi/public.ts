...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for axon serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(AxonHttpApi)
...
export const PublicApi = AxonHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "axon",
    version: "1.0.0",

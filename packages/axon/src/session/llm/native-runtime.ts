...
import type { LLMClientShape } from "@axon-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("axon"))
    return { type: "unsupported", reason: "provider is not openai, axon, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @axon-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains axon-owned. The native runtime only adapts
      // the @axon-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({

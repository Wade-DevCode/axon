...
import type { Message, AxonClient, SessionMessageResponse } from "@axon-ai/sdk/v2"
...
export const AuthMethodID = "axon-login"
...
export class Service extends Context.Service<Service, Interface>()("@axon/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `axon auth login` in the terminal",
      name: "Login with axon",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: AxonClient) {
...
function makeUsageService(sdk: AxonClient) {
...
async function loadDirectorySnapshot(sdk: AxonClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, axon provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const axonProvider = providers[ProviderV2.ID.make("axon")]
  const axonModel = axonProvider ? Provider.sort(Object.values(axonProvider.models))[0] : undefined
  if (axonProvider && axonModel) return { providerID: axonProvider.id, modelID: axonModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "Axon service failure", service })
}

...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@axon-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { errorData, errorMessage } from "@axon-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@axon-ai/tui/context/theme"
import { Global } from "@axon-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@axon-ai/core/util/flock"
import { Flag } from "@axon-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@axon-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@axon-ai/core/v1/config/plugin"
import { createCommandShim } from "@axon-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@axon-ai/tui/plugin/runtime"
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".axon", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.AXON_PURE ? [] : pluginOrigins
    if (Flag.AXON_PURE && pluginOrigins.length) {
    }

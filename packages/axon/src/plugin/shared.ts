...
export const DEPRECATED_PLUGIN_PACKAGES = ["axon-openai-codex-auth", "axon-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, axonVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(axonVersion) || semver.major(axonVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.axon
  if (typeof range !== "string") return
  if (!semver.satisfies(axonVersion, range)) {
    throw new Error(`Plugin requires axon ${range} but running ${axonVersion}`)
  }
}

...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@axon/acme")).toBe("@axon/acme")
    expect(Npm.sanitize("@axon/acme@1.0.0")).toBe("@axon/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/axon/acme.git"
    const expected = win ? "acme@git+https_//github.com/axon/acme.git" : spec

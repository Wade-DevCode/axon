...
import { ConfigProvider } from "@axon-ai/core/config/provider"
import { ConfigMigrateV1 } from "@axon-ai/core/v1/config/migrate"
...
describe("Config", () => {
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "axon.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "axon.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "axon.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "axon.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "axon.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .axon configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".axon"), { recursive: true })
            await fs.mkdir(path.join(directory, ".axon"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "axon.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "axon.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "axon.json"), JSON.stringify({ $schema: "root" })),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".axon")),
              AbsolutePath.make(path.join(directory, ".axon")),
            ])

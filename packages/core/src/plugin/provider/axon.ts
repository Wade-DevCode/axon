...
import type { IntegrationOAuthMethodRegistration } from "@axon-ai/plugin/v2/effect/integration"
import { define } from "@axon-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@axon-ai/sdk/v2/types"
...
export const AxonPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "axon",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("AxonPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("axon")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Axon provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("axon", (integration) => {
        integration.name = "Axon"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "axon", method: { type: "key", label: "API key (service account)" } })
    })
    ...
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("axon")
          if (item.name !== undefined) provider.name = item.name.replace(/Axon/g, "Axon")
      ...
      const item = catalog.provider.get(ProviderV2.ID.axon)
      if (!item) return
      const hasKey = Boolean(process.env.AXON_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("axon")),

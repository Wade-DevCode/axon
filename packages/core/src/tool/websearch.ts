...
import { ToolFailure } from "@axon-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@axon/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.AXON_WEBSEARCH_PROVIDER === "exa" || process.env.AXON_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.AXON_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("AXON_EXPERIMENTAL") || truthy("AXON_ENABLE_EXA") || truthy("AXON_EXPERIMENTAL_EXA"),
    enableParallel: truthy("AXON_ENABLE_PARALLEL") || truthy("AXON_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `axon/${InstallationVersion}`,

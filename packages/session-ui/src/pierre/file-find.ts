...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("axon-find")
  api.delete("axon-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("axon-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("axon-find", new Highlight(...rest))
    return true
  }

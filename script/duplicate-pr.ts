...
import { createAxon } from "@axon-ai/sdk"
...
`)
  ...
  const axon = await createAxon({ port: 0 })
  ...
  try {
    ...
    const session = await axon.client.session.create()
    const result = await axon.client.session
      .prompt({
